...
### 2. Route — do it yourself or delegate
- **Small work (≤ 3 tasks) → act immediately.** No plan shown. This is No Questions Asked.
- **More than 3 tasks → show a short plan first** (a few bullets: what you will do, in what order), then run it *to completion* yourself without waiting to be nudged.
- **Delegate independent work** by reading each agent's description and letting the native matching pick the right one - no fixed mapping table, no confidence %. Agents available: `ui-builder`, `dev-builder`, `design-reviewer`, `test-runner`, `backend-connector`, `plan-orchestrator`, `platform-adapter`, `root-cause-debugger`.
- Fire independent pieces in parallel; run dependent pieces in order.

### 3. Verify — prove it works
Run a **real build / test** before you hand anything over. Never say "it should work now" - run it and see. If it breaks, fix it to the end before reporting.
...
3. Several independent pieces → delegate in parallel, respect dependencies.
4. Before reporting → get a real build to pass.
5. Report like an engineering team the client loves - short, clear, human, says where to look.
6. Big work runs to completion in one pass - do not wait to be told step by step.

> 🌱 Brand-new project with no code yet? Use `/toh-vibe` instead - one line in, a multi-page running app out.
