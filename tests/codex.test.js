 ...
 *
 * Covers:
 *   - fresh installation layout (.toh/, .codex/skills/, AGENTS.md)
...
test('fresh install creates .toh/, .codex/skills/ and AGENTS.md', async () => {
  ...
  try {
    ...
    for (const skill of EXPECTED_SKILLS) {
      assert.ok(
        await fs.pathExists(path.join(dir, '.codex', 'skills', skill, 'SKILL.md')),
...
test('unrelated user Codex skills are never touched', async () => {
  const dir = await makeTmpProject();
  try {
    const userSkillDir = path.join(dir, '.codex', 'skills', 'my-company-skill');
    ...
    // Even a user-owned skill with a toh- name survives (no generator marker).
    const userTohDir = path.join(dir, '.codex', 'skills', 'toh-custom');
...
test('stale TOH-managed skills are removed on reinstall', async () => {
  ...
  try {
    ...
    // Simulate a skill generated by an older TOH version (has the marker).
    const staleDir = path.join(dir, '.codex', 'skills', 'toh-legacy');
    ...
    assert.ok(await fs.pathExists(path.join(dir, '.codex', 'skills', 'toh-plan', 'SKILL.md')), 'current skills intact');
  } finally {
    await fs.remove(dir);
  }
});
...
test('uninstall removes TOH Codex files and keeps user files + .toh state', async () => {
  ...
  try {
    ...
    const { removedSkills, agentsMd } = await uninstallCodex(dir);

    assert.equal(removedSkills.length, EXPECTED_SKILLS.length, 'all TOH skills removed');
    for (const skill of EXPECTED_SKILLS) {
      assert.ok(!(await fs.pathExists(path.join(dir, '.codex', 'skills', skill))), `removed ${skill}`);
    }
    assert.ok(await fs.pathExists(path.join(userSkillDir, 'SKILL.md')), 'user skill remains');
    assert.ok(await fs.pathExists(path.join(dir, '.codex', 'skills')), '.codex/skills dir kept');
...
test('every generated SKILL.md is valid and its references resolve', async () => {
  ...
  try {
    ...
    const skillsRoot = path.join(dir, '.codex', 'skills');
    const entries = (await fs.readdir(skillsRoot, { withFileTypes: true })).filter((e) => e.isDirectory());
    assert.equal(entries.length, EXPECTED_SKILLS.length, 'exactly the TOH skills exist');
  ...
  try {
    ...
    for (const entry of catalog) {
      const raw = await fs.readFile(
        path.join(dir, '.codex', 'skills', entry.skillName, 'SKILL.md'),
        'utf8'
      );
