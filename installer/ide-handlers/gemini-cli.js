...
export async function setupGeminiCLI(targetDir, srcDir, language = 'en') {
  ...
  // Refresh only a file we generated (recognized by our own first line) —
  // a user-authored GEMINI.md is never overwritten (reinstall contract,
  // mirrors the .claude/loop.md marker guard).
  const geminiMdPath = path.join(geminiDir, 'GEMINI.md');
  const geminiMd = language === 'th' ? generateGeminiMdTH() : generateGeminiMdEN();
  let writeGeminiMd = true;
  if (await fs.pathExists(geminiMdPath)) {
    const existingMd = await fs.readFile(geminiMdPath, 'utf-8');
    const firstLine = existingMd.split(/\r?\n/, 1)[0].trim();
    if (firstLine !== GEMINI_MD_HEADER) {
      writeGeminiMd = false;
      console.warn('  ⚠️  .gemini/GEMINI.md was not generated by toh-framework — left untouched.');
    }
  }
  if (writeGeminiMd) await fs.writeFile(geminiMdPath, geminiMd);

  // Create settings.json - Simplified since skills are auto-discovered.
  // .gemini/settings.json is the user's Gemini CLI config surface (auth,
  // mcpServers, tool settings) — NEVER overwrite an existing one (same
  // contract as .codex/config.toml); seed it only when absent.
  const settingsPath = path.join(geminiDir, 'settings.json');
  if (await fs.pathExists(settingsPath)) {
    console.warn('  ⚠️  .gemini/settings.json already exists — left untouched (user Gemini CLI config is never overwritten).');
  } else {
    const settings = {
...
// First line of every GEMINI.md this handler has ever generated — the
// ownership mark that lets a reinstall refresh OUR file while a
// user-authored GEMINI.md (different first line) is never overwritten.
const GEMINI_MD_HEADER = '# Toh Framework - Gemini CLI Integration';
